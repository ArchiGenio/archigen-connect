# ArchiGen Connect

## AI Agent for AEC

ArchiGen Connect connects compatible AI assistants with AEC design applications, helping you work with Rhino, Grasshopper, Revit workflows, and project context from one local setup.

**Beta**

**Current version: 0.1.0-beta.1**

```text
AI Assistant -> ArchiGen Connect -> Rhino / Grasshopper / AEC workflows
```

### Beta capabilities

- AI-assisted Grasshopper workflows
- Rhino connectivity
- Grasshopper definition inspection and modification
- Parametric workflow assistance
- Project-aware workflows
- Copilot-compatible workflow
- Foundation for Rhino.Inside.Revit and Revit workflows

## Download ArchiGen Connect

Download the first public Beta from the official GitHub Release:

**[Download ArchiGen Connect 0.1.0-beta.1](https://github.com/ArchiGenio/archigen-connect/releases/tag/v0.1.0-beta.1)**

- **Version:** `0.1.0-beta.1`
- **Platform:** Windows
- **Installer:** `ArchiGenConnectSetup-0.1.0-beta.1.exe`
- **SHA-256:** `36DC7762F432933561668DF7DC8D92F4A2F8A9CFD5FEC668DB7274924CC0D7BA`

See [INSTALL.md](INSTALL.md) before starting setup. For supported environments, see [REQUIREMENTS.md](REQUIREMENTS.md).

## Support

ArchiGen Connect is in Beta. Read [TROUBLESHOOTING.md](TROUBLESHOOTING.md) for common setup issues, then [open an issue](https://github.com/ArchiGenio/archigen-connect/issues/new/choose) if you need help.

Please do not include passwords, tokens, API keys, confidential project files, or private company information in an issue.

## Documents

- [Installation](INSTALL.md)
- [Requirements](REQUIREMENTS.md)
- [Troubleshooting](TROUBLESHOOTING.md)
- [Changelog](CHANGELOG.md)
- [Beta feedback thread](https://github.com/ArchiGenio/archigen-connect/issues/1)
